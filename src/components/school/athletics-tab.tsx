...
export function AthleticsTab({
  sports,
  selectedSportId: externalSelectedSportId,
  onSportChange: externalOnSportChange,
  stadiumCapacity,
  traditions,
  gameDayDescription,
  colorPrimary,
  ...
  useTrackDuration('athletics', schoolId)
  const [showAllCoaches, setShowAllCoaches] = useState(false)

  // Sort sports alphabetically, default to first
  const sortedSports = [...sports].sort((a, b) => a.name.localeCompare(b.name))

  // Internal state fallback when not controlled externally
  const [internalSportId, setInternalSportId] = useState<string | null>(sortedSports[0]?.id || null)
  const selectedSportId = externalSelectedSportId !== undefined ? externalSelectedSportId : internalSportId
  const onSportChange = externalOnSportChange || setInternalSportId

  const activeSport = sortedSports.find(s => s.id === selectedSportId) || sortedSports[0]

  if (!activeSport) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <p className="text-sm text-muted-foreground">No athletic programs available.</p>
      </div>
    )
  }

  const headCoach = activeSport.coaches.find(c => !!c.careerRecord)
  const coordinators = activeSport.coaches.filter(c => !c.careerRecord && (c.title.includes('Coordinator') || c.title.includes('Associate Head')))
  const positionCoaches = activeSport.coaches.filter(c => !c.careerRecord && !c.title.includes('Coordinator') && !c.title.includes('Associate Head'))
  const visibleCoaches = showAllCoaches ? positionCoaches : []

  // Use offensive/defensive scheme for football/basketball, playStyle for others
  const hasScheme = activeSport.offensiveScheme || activeSport.defensiveScheme
  const hasPlayStyle = activeSport.playStyle || activeSport.programDescription
  ...
  return (
    <div className="space-y-10 animate-in-up">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-display text-lg tracking-wide text-foreground">{activeSport.name}</h3>
              <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">
                {activeSport.conference}
                {activeSport.division && ` · ${activeSport.division}`}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {activeSport.record && <span className="text-scoreboard text-lg font-bold text-foreground">{activeSport.record}</span>}
            {activeSport.ranking && (
              <Badge style={{ backgroundColor: colorPrimary + '20', color: colorPrimary }} className="font-display text-sm">#{activeSport.ranking}</Badge>
            )}
          </div>
        </div>
      </div>
      ...
      {/* ─── Program Description ────────────────────────────────── */}
      {hasPlayStyle && !hasScheme && (
        <div>
          {activeSport.programDescription && (
            <p className="text-sm text-muted-foreground leading-relaxed mb-4">{activeSport.programDescription}</p>
          )}
          {activeSport.playStyle && (
            <div className="glass-panel rounded-xl p-5">
              <span className="text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground mb-2 block">Play Style</span>
              <p className="text-xs text-muted-foreground leading-relaxed">{activeSport.playStyle}</p>
            </div>
          )}
        </div>
      )}
      ...
      {/* ─── Performance Stats ──────────────────────────────────── */}
      {(activeSport.conferenceStanding || activeSport.recentBowlGames.length > 0) && (
        <div>
          <h4 className="mb-4 text-display text-xs tracking-[0.15em] text-muted-foreground">Performance</h4>
          <div className="flex flex-wrap gap-3">
            {activeSport.conferenceStanding && (
              <div className="glass-panel rounded-xl px-5 py-3">
                ...
                <p className="text-scoreboard text-lg font-bold" style={{ color: colorPrimary }}>{activeSport.conferenceStanding}</p>
              </div>
            )}
            {activeSport.recentBowlGames.length > 0 && (
              <div className="glass-panel rounded-xl px-5 py-3">
                <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground mb-1.5">Recent Achievements</p>
                <div className="space-y-1">
                  {activeSport.recentBowlGames.map((game) => (
      ...
      {/* ─── Scheme Identity (Football/Basketball) ──────────────── */}
      {hasScheme && (
        <div>
          {activeSport.programDescription && (
            <p className="text-sm text-muted-foreground leading-relaxed mb-4">{activeSport.programDescription}</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {activeSport.offensiveScheme && (
              <div className="glass-panel rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground">Offense</span>
                  <span className="rounded-sm px-2 py-0.5 text-xs font-bold uppercase tracking-wider" style={{ backgroundColor: colorPrimary + '15', color: colorPrimary }}>{activeSport.offensiveScheme}</span>
                </div>
                {activeSport.offenseDescription && <p className="text-xs text-muted-foreground leading-relaxed">{activeSport.offenseDescription}</p>}
              </div>
            )}
            {activeSport.defensiveScheme && (
              <div className="glass-panel rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground">Defense</span>
                  <span className="rounded-sm px-2 py-0.5 text-xs font-bold uppercase tracking-wider" style={{ backgroundColor: colorPrimary + '15', color: colorPrimary }}>{activeSport.defensiveScheme}</span>
                </div>
                {activeSport.defenseDescription && <p className="text-xs text-muted-foreground leading-relaxed">{activeSport.defenseDescription}</p>}
              </div>
            )}
          </div>
        </div>
              ...
              <div className="mt-4 space-y-3 border-t border-white/[0.06] pt-4">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {headCoach.playersInNfl != null && headCoach.playersInNfl > 0 && (
                    <Badge style={{ backgroundColor: colorPrimary + '15', color: colorPrimary }} className="text-[10px] uppercase tracking-wider">{headCoach.playersInNfl} players to pros</Badge>
                  )}
                </div>
          ...
          <>
            ...
            >
              {showAllCoaches ? (
                ...
                <>{positionCoaches.length} more coaches <ChevronDown className="h-3.5 w-3.5" /></>
              )}
            </button>
          </>
        )}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {activeSport.strengthProgram && (
              <div className="glass-panel rounded-xl p-5">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">{activeSport.strengthProgram}</p>
              </div>
            )}
            {activeSport.nutritionProgram && (
              <div className="glass-panel rounded-xl p-5">
                <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">{activeSport.nutritionProgram}</p>
              </div>
            )}
          </div>
        </div>
